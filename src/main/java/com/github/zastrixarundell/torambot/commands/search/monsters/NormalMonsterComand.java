...
public class NormalMonsterComand implements MessageCreateListener
{
    ...
    public void onMessageCreate(MessageCreateEvent messageCreateEvent)
    {
        ...
        if (!messageCreateEvent.getMessageContent().toLowerCase().startsWith(Values.getPrefix() + "normalmonster"))
            if (!messageCreateEvent.getMessageContent().toLowerCase().startsWith(Values.getPrefix() + "nmonster"))
                return;
        ...
        {
            sendError(messageCreateEvent, "Empty Search", "You can't search for a normal monster without specifying which one!");
            return;
        }
        ...
        Runnable runnable = () ->
        {
            ...
            catch (Exception e)
            {
                sendError(messageCreateEvent, "Error while getting the monster!",
                        "An error happened while getting the monster info! Does the specified normal monster even " +
                                "exist?");
            }
        };
